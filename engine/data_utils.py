...
def category_to_number(category):
    cat_dict = {"AnnualCrop":0, "Forest":1, "HerbaceousVegetation":2, \
...
def get_EuroSAT_data(
    num_training=23300, num_validation=1000, num_test=1000, subtract_mean=True
):
    """
    Load the EuroSAT dataset from disk and perform preprocessing to prepare
    it for classifiers. These are the same steps as we used for the SVM, but
    condensed to a single function.
    """
    # Load the raw EuroSAT data
    eurosat_dir = os.path.join(
        os.path.dirname(__file__), "datasets/EuroSAT"
    )
